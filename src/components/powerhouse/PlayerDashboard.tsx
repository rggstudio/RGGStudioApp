...
import GameCard from '@/components/powerhouse/GameCard'
...
const PlayerDashboard = ({ dashboard }: { dashboard: DashboardData }) => {
  ...
  const handlePick = async (gameId: string, selection: 'home' | 'away') => {
    ...
    try {
      const response = await fetch('/api/powerhouse/player/picks', {
        method: 'POST',
        headers: {
  ...
  const handleLogout = async () => {
    await fetch('/api/powerhouse/auth/logout', { method: 'POST' })
