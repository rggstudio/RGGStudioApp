...
import AdminTable from '@/components/powerhouse/AdminTable'
import PointsLedger from '@/components/powerhouse/admin/PointsLedger'
import GameCard from '@/components/powerhouse/admin/GameCard'
import PlayerRequests from '@/components/powerhouse/admin/PlayerRequests'
...
const AdminDashboard = ({ dashboard }: { dashboard: AdminDashboardData }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'games' | 'ledger' | 'player-requests' | 'management'>('overview')
  ...
  const handleSetResult = async (gameId: string, result: 'home' | 'away') => {
    ...
    try {
      const response = await fetch(`/api/powerhouse/admin/games/${gameId}/result`, {
        method: 'POST',
  ...
  const handleToggleLock = async (gameId: string, isLocked: boolean) => {
    ...
    try {
      const response = await fetch(`/api/powerhouse/admin/games/${gameId}/lock`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAdjustPoints = async (teamId: string, points: number, note?: string) => {
    ...
    try {
      const response = await fetch('/api/powerhouse/admin/points', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreateGame = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/powerhouse/admin/games', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateGame = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/powerhouse/admin/games/${editingGame.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteGame = async (gameId: string) => {
    ...
    try {
      const response = await fetch(`/api/powerhouse/admin/games/${gameId}`, {
        method: 'DELETE',
      })
