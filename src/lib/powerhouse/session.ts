...
const SESSION_COOKIE = 'powerhouse.session'
const ADMIN_COOKIE = 'powerhouse.admin'
const SESSION_TTL = 60 * 60 * 24 * 7 // 7 days

const getSessionSecret = () => {
  const secret = process.env.POWERHOUSE_SESSION_SECRET
  if (!secret) {
    throw new Error('POWERHOUSE_SESSION_SECRET is not set')
  }
...
export const setTeamSession = async (teamId: string) => {
  ...
  // Clear legacy cookie scoped to /powerhouse
  cookieStore.set({
    name: SESSION_COOKIE,
    value: '',
    httpOnly: true,
