...
export const getTeamDashboardData = async (teamId: string) => {
  ...
  const [
    ...
    supabase.from('phl_teams').select('id, name, short_code').eq('id', teamId).single(),
    supabase.from('phl_team_points').select('team_id, total_points').eq('team_id', teamId).maybeSingle(),
    supabase.from('phl_weeks').select('week_number, label'),
    supabase
      ...
      .select(`
        ...
        game_id,
        team:phl_teams(name),
        game:phl_games(title, home_team, away_team, week_number)
      `)
      .eq('team_id', teamId)
  ...
  let admins: any[] = []
  try {
    const { data: adminData } = await supabase.from('phl_admins').select('id, email')
    admins = adminData || []
  } catch {
  ...
  const { data: games, error: gamesError } = await supabase
    ...
    .select(
      `
        ...
        is_locked,
        result,
        picks:phl_picks(team_id, selection, updated_at)
      `,
    )
...
export const getAdminDashboardData = async () => {
  ...
  const [{ data: games }, { data: teams }, { data: points }, { data: weeks }, { data: ledger }] = await Promise.all([
    supabase
      ...
      .order('week_number', { ascending: true }),
    supabase.from('phl_teams').select('id, name, short_code').order('name', { ascending: true }),
    supabase.from('phl_team_points').select('team_id, total_points'),
    supabase.from('phl_weeks').select('week_number, label'),
    supabase
      .from('phl_points_ledger')
      .select(`
