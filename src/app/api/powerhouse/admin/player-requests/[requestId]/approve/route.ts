...
export const POST = async (
  ...
  // Deduct points from team (negative entry)
  const { error: deductError } = await supabase.from('phl_points_ledger').insert({
  ...
  if (deductError) {
    // Rollback the status update if ledger insert fails
    await supabase
      .from('phl_player_requests')
